...
export default class MasonryList extends React.PureComponent {
  ...
  static propTypes = {
    ...
    masonryListRef: PropTypes.object,
    ...
    customImageComponent: PropTypes.oneOfType([PropTypes.func, PropTypes.node]),
    customImageProps: PropTypes.object,
    completeCustomComponent: PropTypes.oneOfType([PropTypes.func, PropTypes.node]),
  ...
  UNSAFE_componentWillReceiveProps = (nextProps) => {
    if (
      nextProps.layoutDimensions.width &&
      nextProps.layoutDimensions.height &&
      nextProps.layoutDimensions.columnWidth &&
      nextProps.layoutDimensions.gutterSize &&
    ...
    if (!this.props.rerender) {
      ...
      if (nextProps.images.length > this.props.images.length) {
        let newImages = nextProps.images
          .concat()
          .splice(this.props.images.length, nextProps.images.length); // nextProps.images
        this.resolveImages(
          nextProps.itemSource,
          newImages,
          nextProps.layoutDimensions,
  ...
  _getCalculatedDimensions(
    ...
    const newHeight =
      countDecimals(tempHeight) > 10 ? parseFloat(tempHeight.toFixed(10)) : tempHeight;
  ...
  resolveImages(
    ...
    function _assignColumns(image, nColumns) {
      ...
      if (!resolverObj.columnHeightTotals[resolverObj.columnCounting - 1]) {
        ...
        resolverObj.columnHeightTotals[resolverObj.columnCounting - 1] =
          resolverObj.columnHeightTotals[resolverObj.columnCounting - 1] + height;
      }

      if (!resolverObj.columnHighestHeight) {
        resolverObj.columnHighestHeight =
          resolverObj.columnHeightTotals[resolverObj.columnCounting - 1];
        resolverObj.columnCounting =
          resolverObj.columnCounting < nColumns ? resolverObj.columnCounting + 1 : 1;
      } else if (
        resolverObj.columnHighestHeight <=
        resolverObj.columnHeightTotals[resolverObj.columnCounting - 1]
      ) {
    ...
    if (images && itemSource.length > 0) {
          ...
          (resolvedImages) => {
            resolvedImages.map((resolvedData, index) => {
              ...
              if (this.props.onImageResolved) {
                finalizedData =
                  this.props.onImageResolved(finalizedData, this.renderIndex) || finalizedData;
              }
      ...
      const resolveImages = images.map((image) => {
        const source = getImageSource(image);
        const uri = getImageUri(image);
        ...
        sequence(
          ...
          (resolvedImages) => {
            resolvedImages.map((resolvedImage, index) => {
              ...
              if (this.props.onImageResolved) {
                resolvedImage =
                  this.props.onImageResolved(resolvedImage, this.renderIndex) || resolvedImage;
              }
  ...
  render() {
    return (
      <FlatList
        ...
        initialNumToRender={
          this.props.initialColToRender ? this.props.initialColToRender : this.props.columns
        }
