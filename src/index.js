...
import { isReactComponent, isElement } from './utils';
...
class Masonry extends React.PureComponent {
  ...
  UNSAFE_componentWillReceiveProps = (nextProps) => {
    if (!nextProps.containerWidth && !this.props.containerWidth) {
      if (nextProps.columns !== this.props.columns || nextProps.spacing !== this.props.spacing) {
  ...
  render() {
    ...
    return (
      <View
        style={
          !this.props.containerWidth ? { flex: 1 } : { flex: 1, width: this.props.containerWidth }
        }
        onLayout={(event) => {
      ...
      >
        <MasonryList
          ...
          masonryListRef={this.props.masonryListRef}
